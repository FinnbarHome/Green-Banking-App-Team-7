...
async function apiRequest(endpoint, method = "GET", body = null) {
  const url = `${API_BASE_URL}${endpoint}`; // Prefix API calls with the backend URL
  const options = {
    method,
    headers: {
...
function createDiscountElement(discount) {
  const discountElement = createElement("div", [
    "flex",
    "justify-between",
    "items-center",
    "bg-green-900",
  ...
  const companyElement = createElement(
    "h2",
  ...
  const descriptionElement = createElement(
    "h2",
    ["text-xl", "text-white", "text-right", "text-red-400"],
...
async function fetchDiscounts() {
  try {
    ...
    if (discounts.length === 0) {
      rewardsContainer.innerHTML =
        '<p class="text-xl font-bold text-white text-center">No discounts available at the moment.</p>';
      return;
    }

    // Show each discount
    discounts.forEach((discount) =>
      rewardsContainer.appendChild(createDiscountElement(discount))
    );
  } catch (error) {
...
async function createDiscount() {
  ...
  try {
    await apiRequest("/discounts", "POST", {
...
async function deleteDiscount(discountID) {
  try {
    await apiRequest(`/discounts/${discountID}`, "DELETE");
    fetchDiscounts();
  } catch (error) {
...
document
  .getElementById("createDiscountButton")
  .addEventListener("click", createDiscount);
